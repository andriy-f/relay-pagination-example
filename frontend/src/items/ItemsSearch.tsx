...
import type { ItemsSearchFragment$key } from "./__generated__/ItemsSearchFragment.graphql"
import Item from "./item"

const itemsSearchFragment = graphql`
  fragment ItemsSearchFragment on Query
  @refetchable(queryName: "ItemsSearchRefetchQuery")
  @argumentDefinitions(
    search: { type: String },
...
const Items = ({ itemsRef }: { itemsRef: ItemsSearchFragment$key }): React.ReactNode => {
  const [pageSize] = useState(5)
  const [queryRes, refetch] = useRefetchableFragment(itemsSearchFragment, itemsRef)
